...
- **Offline-first**: `sw.js` (HTML network-first **con timeout de 3 s** —
  con señal mala se abre desde el cache en vez de colgarse —, resto
  cache-first, con versión de cache que se sube en cada release) +
  `manifest.webmanifest` para instalarla en el teléfono. Las librerías de
  CDN se cargan bajo demanda: nada externo bloquea el arranque.
...
| 17 | **PDF de ejercicio y de sesión** (📄 en listas y editores): ejercicio con la cancha dibujada (1 paso grande o secuencia "PASO 1..8" si hay animación, rasterizada con `pzBoardPng`/`pzCanvasDraw`), dosificación, densidad, planificación, descripción, materiales y participantes; sesión con bloques, miniatura + minutos por ítem, materiales juntos y notas. Helpers `_pdfHead`/`_pdfFoot`/`_pdfExerciseBody` en `js/pdf.js` | HECHA (2026-07-12) |
...
| 22 | **Pizarra — líneas/flechas y guías**: elementos `board.lines` (→ flecha, ⇢ punteada, ─ línea) arrastrables enteros o por extremos (manijas), con color ciclado; guías superpuestas `board.guide` cicladas con ▦ (cuadrícula 5×7, tercios, carriles), presentes también en video y PDF | HECHA (2026-07-12) |
...
| 27 | **Pizarra — pasos al final + deshacer + versiones** (pedidos 2026-07-13): "➕ paso" agrega SIEMPRE al final copiando el último (antes en ejercicios guardados quedaba al principio); pila ↩ Deshacer (`pzUndo`, 40 niveles, 1 por gesto/acción, solo sesión de edición); al guardar, la pizarra anterior queda como versión restaurable (`e.hist`, máx 5, chip "🕘 Versión anterior") por si se guardó por error — `hist` no viaja al compartir/duplicar | HECHA (2026-07-13) |
| 28 | **Pizarra — 4 equipos**: además de 🔴 rojo y 🔵 azul, fichas 🟠 naranja y ⚪ blanco (número/borde oscuros para leerse sobre el pasto); "Ponerlos en la cancha" con los 4 colores (naranjas arriba, blancos abajo) | HECHA (2026-07-13) |
